        ...
        while (running) {
            ...
            if (!job?.id) {
                ...
                try {
                    ...
                    await JobModel.transaction(async (trx: any) => {
                        const released: number = await trx
                            .query()
                            .where("id", job.id)
                            .where("reserved_at", claimStamp)
                            .update({
