...
export default class JobModel extends EpochTimestamps(BaseModel) {
    ...
    public static async claim(
        criteria: (builder: any) => any,
        claimedAt: number,
        retries: number = 5
    ): Promise<JobModel | undefined> {
        for (let i = 0; i < retries; i++) {
            const candidate: any = await criteria(JobModel.query().select("id", "reserved_at"))
                .orderBy("id", "asc")
                .limit(1)
                .first();
            ...
            const attempt: any = criteria(JobModel.query().where("id", candidate.id));
            ...
            const claimedCount: number = await attempt.patch({reserved_at: claimedAt});

            if (claimedCount === 1) return JobModel.query().findById(candidate.id);
